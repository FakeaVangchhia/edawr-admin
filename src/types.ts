...
/**
 * Where a rider was, the last time their phone said anything.
 *
 * From `GET /api/delivery/locations`, which is built from the rider roster
 * outward — so a rider who has never reported still appears, with every
 * position field null and `is_stale` true. That is deliberate: a map that
 * silently omits three riders cannot be told apart from a map of a quiet shift.
 *
 * **`age_seconds` is the server's own arithmetic, and the reason to prefer it
 * over subtracting `received_at` from the browser's clock.** A console running
 * on a laptop whose clock is four minutes out would otherwise render every
 * rider as four minutes stale.
 *
 * There is no `base_latitude` here on purpose. That is the rider's home address
 * and it belongs to the staff editor that sets it, not to a live view.
 */
export interface RiderLocation {
  ...
  /** Past `LOCATION_STALE_SECONDS` on the API. Shown, never hidden — see below. */
  is_stale: boolean;
  /** The delivery they are on, for joining to the orders board. */
