---
name: run-project
description: Start eDawr locally — Django API on 8000, storefront on 3000, admin console on 3001, Expo for the rider app — including the PostgreSQL the API now requires, and the store-hours setting that silently blocks checkout outside 07:00–22:00. Use when asked to run, start, serve or demo the project, when a package starts but shows no data, or when checkout refuses an order that looks fine.
...
```bash
until curl -sf -o /dev/null http://127.0.0.1:8000/api/health \
   && curl -sf -o /dev/null http://127.0.0.1:3000 \
   && curl -sf -o /dev/null http://127.0.0.1:3001/login; do sleep 1; done
```
...
The dev servers hold the database connections the test suite wants, so stop them
before a long test run:

```bash
powershell -NoProfile -Command "Get-CimInstance Win32_Process | Where-Object { \$_.CommandLine -like '*runserver*' -or \$_.CommandLine -like '*next dev*' } | ForEach-Object { Stop-Process -Id \$_.ProcessId -Force }"
```
